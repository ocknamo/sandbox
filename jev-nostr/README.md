...
| `topic` | `choice` | 何について書かれているか（22トピック） |

肯定シグナルを3つに分けてあるのは、**投稿はそのうちどれか1つで十分に価値を持つ**
からです。ダジャレに洞察を求めても仕方がなく、ぼやきに情報量を求めても仕方がない。
単一の「良い投稿か？」ではこの区別ができません。

**8つ聞いてもコストはほとんど増えません。** 1リクエストの入力トークンの多くが固定費で、
質問は並列に評価されます。高いのは**質問の数ではなくリクエストの数**です。

`language` と `topic` は `choice` ひとつで済ませています。**欲しいのは勝った選択肢ではなく
確率分布のほう**で、`probabilities["ja"]` がそのまま「日本語らしさ」になります。
選択肢ごとに `noul` を並べると選択肢の数だけ質問が要りますが、`choice` なら1問で全部を
カバーでき、しかも**読み手が後から選び直しても再判定が不要**です。

### トピックは足し算で読みます

`choice` の確率は**合計が 1** なので、2つのトピックに跨る投稿は薄まります。

```
「Go でスポーツの試合データを分析した」 → tech 0.45 / sports 0.40 / ...
```

しきい値 0.5 ではどちらも越えません。そこで**選んだトピックの確率を足します**。
tech と sports の両方に興味があるなら **0.85**。合計が 1 の分布なので、これは
そのまま「この投稿が私の興味のどれかに当たる確率」になります。単独のスコアに
しきい値を越えさせようとすると、**複数トピックに跨る投稿が軒並み落ちます**。
面白いのはたいていそういう投稿です。

トピックは 22 個の**フラットな一覧**です。`choice` は 255 個まで受け付け、
ドキュメントも「短縮リストではなく完全なリストを渡せ」「選択肢の追加コストは数トークン」
と書いています。階層分類が要るのは数千ノード規模の分類体系の話です。

紛らわしい選択肢には `what` / `not_for` / `examples` を持つ**構造化した説明**を
使っています。`tech` / `bitcoin` / `nostr` は `not_for` が無いと `tech` が
他の2つを飲み込みます。
...
```text
asking "jev-latest" 8 questions about 5 posts (appeal >= 0.50, promotional veto >= 0.50)
...
{"results": [{"id": "…", "insight": 0.90, "humor": 0.09, "relatable": 0.44,
              ...
              "topic": "tech", "topic_confidence": 0.45,
              "topic_scores": {"tech": 0.45, "nostr": 0.35, "science": 0.05}}]}
```
...
`language_scores` と `topic_scores` も同じ理由で分布ごと返しています。
**どの言語・どのトピックのタイムラインが欲しいかは投稿の性質ではなく読み手の都合**
なので、サーバが決めてしまうと読み手が気を変えるたびに聞き直すことになります。

興味のあるトピックはチップで複数選択でき、選んだものが**レーダーチャートの軸**に
なります。レーダーは長さより形で読む図なので棒グラフより精度は落ち、面積が軸の
並び順に依存します。そこを抑えるため、**軸の順序は選択順で固定**（カード間で比較できる）、
**目盛りは投稿ごとに正規化せず 0〜1 固定**（薄い投稿は薄いまま見える）にしてあります。
フィルタが実際に使う合計値は図の下に数字で出します。軸が2つ以下のときは多角形に
ならないので棒に落とします。

リレーには `limit: 100` を投げています。フィルタが厳しいほど母数が要りますが、
**1投稿 = 1リクエスト（有料）**なので、接続1回でおよそ 100 回の呼び出しです。
その後もライブの投稿が流れ込んで積み上がります。
...
3. ~~スコアリング API と、リアルタイムに判定する Web ページ~~
4. ~~言語フィルタとトピックフィルタ~~ ← 今ここ
5. `relatable` の作り直し（下記）
6. `substance` を判定に組み込むかを決める
7. `confidence` によるルーティング、キャッシュ
